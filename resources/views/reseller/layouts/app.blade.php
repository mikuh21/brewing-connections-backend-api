    ...
    <style>
        ...
        *::-webkit-scrollbar-thumb:hover {
        ...
        @media (max-width: 767px) {
            ...
            .reseller-sidebar > div:first-child {
                min-height: 0;
            ...
            .reseller-sidebar-open .reseller-sidebar {
                transform: translateX(0);
            }

            .reseller-sidebar-open .reseller-sidebar-overlay {
                opacity: 1;
                pointer-events: auto;
            }

            .reseller-sidebar-open .reseller-sidebar-toggle {
                opacity: 0;
                pointer-events: none;
                transform: translateY(-8px) scale(0.95);
            }

            .reseller-modal-open .reseller-sidebar-toggle {
                z-index: 30 !important;
            }

            .logout-modal-open .reseller-sidebar {
                z-index: 20 !important;
            }

            .logout-modal-open .reseller-sidebar-overlay {
                z-index: 15 !important;
            }

            .logout-modal-open .reseller-sidebar-toggle {
                z-index: 25 !important;
            }

            .logout-modal-open [data-reseller-logout-modal='container'] {
                z-index: 4000 !important;
            }

            .logout-modal-open [data-reseller-logout-modal='overlay'] {
                z-index: 4000 !important;
            }

            .logout-modal-open [data-reseller-logout-modal='card'] {
                z-index: 4001 !important;
            }

            .reseller-sidebar ~ main {
            ...
            .reseller-sidebar-open body,
            .reseller-sidebar-open {
                overflow: hidden;
            }
        }
    </style>
...
<body class="bg-[#F3E9D7] text-[#3A2E22]">
    ...
    <div class="min-h-screen flex" x-data="{ logoutModalOpen: false }" :class="{ 'logout-modal-open': logoutModalOpen }" @keydown.escape.window="logoutModalOpen = false">
        <aside class="reseller-sidebar fixed left-0 top-0 h-screen w-64 min-w-64 max-w-64 bg-[#3A2E22] text-[#F5F0E8] flex flex-col justify-between py-6 px-4 rounded-r-xl shadow-lg overflow-hidden z-40 -translate-x-full md:translate-x-0 transition-transform duration-300 ease-out">
            <div>
        ...
        <div class="fixed inset-0 flex items-center justify-center px-4" data-reseller-logout-modal="container" x-show="logoutModalOpen" @click="logoutModalOpen = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" style="display: none; z-index: 3000;">
            <div class="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm" data-reseller-logout-modal="overlay" style="z-index: 3000;" @click.stop="logoutModalOpen = false"></div>

            <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full" data-reseller-logout-modal="card" style="z-index: 3001;" @click.stop>
                <div class="p-6">
    ...
    <script>
        (function () {
            ...
            const sidebar = document.querySelector('.reseller-sidebar');
            let closeSidebar = () => {};

            if (sidebar) {
                const toggle = document.createElement('button');
                toggle.type = 'button';
                toggle.className = 'reseller-sidebar-toggle md:hidden';
                ...
                overlay.className = 'reseller-sidebar-overlay md:hidden';
                overlay.setAttribute('aria-hidden', 'true');

                const syncSidebarState = () => {
                    const isOpen = root.classList.contains('reseller-sidebar-open');
                    toggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
                    toggle.setAttribute('aria-label', isOpen ? 'Close navigation menu' : 'Open navigation menu');
                };

                const closeSidebarHandler = () => {
                    root.classList.remove('reseller-sidebar-open');
                    syncSidebarState();
                };
                closeSidebar = closeSidebarHandler;

                const toggleSidebar = () => {
                    root.classList.toggle('reseller-sidebar-open');
                    syncSidebarState();
                };
            ...
            const setScrollLocked = (shouldLock) => {
                if (shouldLock === isScrollLocked) {
                    root.classList.toggle('reseller-modal-open', shouldLock);
