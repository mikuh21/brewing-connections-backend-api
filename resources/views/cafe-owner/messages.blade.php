            ...
            <div class="cafe-conversations-panel w-80 bg-white rounded-xl shadow-sm flex flex-col overflow-hidden">
                ...
                <div class="flex-1 overflow-y-auto">
                    @forelse($conversations as $conv)
                        ...
                        <a
                            href="{{ route('cafe-owner.messages.show', $conv) }}"
                    ...
                    @forelse($users as $user)
                        <div class="user-item" x-show="!userSearch || '{{ strtolower($user->name) }}'.includes(userSearch.toLowerCase())">
                            <button
                                type="button"
                                @click="
                                    fetch('{{ route('cafe-owner.messages.store') }}', {
                                        method: 'POST',
                                    ...
                                    .then(data => {
                                        ...
                                        window.location.href = '{{ url('cafe-owner/messages') }}/' + data.conversation_id;
                                    })
                                    .catch(() => {
...
<script>
    ...
    document.getElementById('message-form')?.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('{{ isset($conversation) && $conversation ? route('cafe-owner.messages.send', $conversation) : '#' }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
