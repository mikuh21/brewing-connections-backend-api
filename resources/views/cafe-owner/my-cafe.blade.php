...
@section('title', 'My Cafe - BrewHub')

@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-display font-bold text-[#3A2E22] mb-1">
        Cafe <span class="italic text-[#4A6741]">Profile</span>
    </h1>
    <p class="text-[#9E8C78] text-sm font-medium">Manage your cafe details</p>
</div>
        ...
        <div class="cafe-profile-hero bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            >
                <template x-if="imagePreviewUrl">
                    <img :src="imagePreviewUrl" alt="Cafe cover" class="h-full w-full object-cover" :style="fitStyle(bannerFitX, bannerFitY)" />
                </template>
            </div>
            ...
            <div class="cafe-profile-hero-body px-8 pb-8 pt-4">
                <div class="cafe-profile-hero-top flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                    <div class="cafe-profile-hero-identity flex items-start gap-4">
                        ...
                        >
                            <template x-if="imagePreviewUrl">
                                <img :src="imagePreviewUrl" alt="Cafe" class="absolute inset-0 w-full h-full max-w-none object-cover" :style="fitStyle(profileFitX, profileFitY)" />
                            </template>
                            <template x-if="!imagePreviewUrl">
                                Cafe Image
                            </template>
                        ...
                        <div class="pt-4">
                            <template x-if="!editMode">
                                <div>
                                    <h2 class="text-3xl font-display font-bold text-[#3A2E22]">{{ $establishment?->name ?: 'Unnamed Cafe' }}</h2>
                                    <p class="text-sm text-[#9E8C78] mt-1">{{ $establishment?->address ?: 'No address yet' }}</p>
                                </div>
                            </template>
                            <template x-if="editMode">
                                <div class="space-y-3">
                                    <input name="name" value="{{ old('name', $establishment?->name) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#4A6741]" placeholder="Cafe name" required />
                    ...
                    <div class="cafe-profile-actions flex items-center gap-3 pt-4">
                        <button
                            ...
                            x-text="editMode ? 'Cancel' : 'Edit Cafe'"
                        ></button>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-xl shadow-sm p-8">
                    ...
                    <p class="text-[#9E8C78] text-sm mb-4">Cafe identity and story</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input x-show="editMode" x-cloak value="{{ old('type', $establishment?->type) }}" disabled class="mt-1 w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2 text-[#6B5B4A] shadow-sm cursor-not-allowed" placeholder="farm/cafe/roaster" />
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea x-show="editMode" x-cloak name="description" rows="4" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#4A6741]" placeholder="Tell customers about your cafe...">{{ old('description', $establishment?->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input x-show="editMode" x-cloak type="number" step="any" name="latitude" value="{{ old('latitude', $establishment?->latitude) }}" disabled class="mt-1 w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2 text-[#6B5B4A] shadow-sm cursor-not-allowed" />
                        </div>
                        ...
                        <div>
                            ...
                            <input x-show="editMode" x-cloak type="number" step="any" name="longitude" value="{{ old('longitude', $establishment?->longitude) }}" disabled class="mt-1 w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2 text-[#6B5B4A] shadow-sm cursor-not-allowed" />
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-[#3A2E22]">Operating Hours</label>
                            <template x-if="!editMode">
                                <p class="mt-1 text-sm text-[#6B5B4A]">{{ $establishment?->operating_hours ?? $establishment?->visit_hours ?: 'N/A' }}</p>
                            </template>
                            <input x-show="editMode" x-cloak name="operating_hours" value="{{ old('operating_hours', $establishment?->operating_hours ?? $establishment?->visit_hours) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#4A6741]" placeholder="Mon-Sat, 8:00 AM - 8:00 PM" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm p-8">
                    ...
                    <p class="text-[#9E8C78] text-sm mb-4">Varieties served at your cafe</p>
                ...
                <div class="bg-white rounded-xl shadow-sm p-8">
                    ...
                    <div class="space-y-2 text-sm text-[#6B5B4A]">
                        <p><span class="font-semibold text-[#3A2E22]">ID:</span> {{ $establishment?->id ?: 'N/A' }}</p>
                        <p><span class="font-semibold text-[#3A2E22]">Owner ID:</span> {{ $establishment?->owner_id ?? $establishment?->user_id ?? 'N/A' }}</p>
