...
@section('content')
@php
    $ownerTypeLabel = static function ($ownerType) {
        return match (strtolower((string) $ownerType)) {
            'farm_owner', 'farm owner', 'farm' => 'Farm Owner',
            'cafe_owner', 'cafe owner', 'cafe' => 'Cafe Owner',
            'reseller' => 'Reseller',
            default => 'Seller',
        };
    };
    ...
    <div class="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
            ...
            <p class="text-[#9E8C78] text-sm font-medium">
                {{ $products->total() }} {{ \Illuminate\Support\Str::plural('product', $products->total()) }} in your listings
            </p>
        </div>
...
<div class="mb-4">
    <h2 class="text-xl font-display font-bold text-[#3A2E22]">My Listings</h2>
    <p class="text-[#9E8C78] text-xs">Products you can edit and manage</p>
</div>

<div class="bg-white rounded-xl shadow-sm p-4 mb-6">
    <form method="GET" action="{{ route('reseller.marketplace') }}" class="flex flex-col md:flex-row gap-3 md:items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            <template x-for="product in hiddenProducts()" :key="`hidden-${product.id}`">
                <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition overflow-hidden border border-[#E7DED1]">
                    ...
                    <div class="p-3">
                        <div class="flex items-start justify-between gap-2 mb-1">
                            <h3 class="font-semibold text-sm text-[#2C1A0E] leading-tight truncate" style="font-family: 'Poppins', sans-serif;" x-text="product.name"></h3>
                        ...
                        <p class="text-[11px] text-[#6B5B4A] mb-2">
                            Seller:
                            <span class="font-semibold text-[#3A2E22]" x-text="(String(product.seller_type || '').toLowerCase() === 'farm_owner' ? 'Farm Owner' : (String(product.seller_type || '').toLowerCase() === 'cafe_owner' ? 'Cafe Owner' : (String(product.seller_type || '').toLowerCase() === 'reseller' ? 'Reseller' : 'Seller')))"></span>
                            -
                            <span class="font-semibold text-[#3A2E22]" x-text="product.seller_name || product.reseller_name || 'Unknown'"></span>
                        </p>
...
@else
    <div class="bg-white rounded-xl shadow-sm p-10 text-center mb-8">
    ...
    @if(($productRatings ?? collect())->count() > 0)
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            @foreach($productRatings as $rating)
                @php
                    $ratingImageUrl = $rating->image_url;
