...
@section('title', 'Cafe Owner Dashboard - BrewHub')

@section('content')
@php
    $cafeName = auth()->user()->name ?? 'Cafe Owner';
...
<div class="cafe-dashboard-header flex items-center justify-between mb-8 sticky top-0 z-10 bg-[#F5F0E8]">
    <div>
        <h1 class="text-3xl font-display font-bold text-[#3A2E22] mb-1">
            Welcome back, <span class="italic text-[#4A6741]">{{ $cafeName }}</span>
        </h1>
        <p class="text-[#9E8C78] text-sm font-medium">Manage your cafe insights and operations</p>
    </div>

    <div
        class="cafe-dashboard-notif flex items-center space-x-4"
        x-data="{
            open: false,
            unreadCount: 0,
            items: [],
            ...
            storageKey() {
                return 'cafeOwnerSeenNotifications:{{ auth()->id() }}';
            },
            loadSeen() {
                try {
            ...
            async fetchNotifications() {
                try {
                    const response = await fetch('{{ route('cafe-owner.notifications') }}', {
                        headers: {
                    ...
                    window.dispatchEvent(new CustomEvent('cafe-owner-notifications-updated', {
                        detail: {
                            items: this.items
                        }
                    }));
    ...
    >
        <div class="relative" @click.away="open = false">
            ...
            <div x-show="open" x-cloak class="cafe-dashboard-notif-menu absolute right-0 mt-2 w-80 bg-white rounded-xl border border-gray-200 shadow-lg z-50 overflow-hidden">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-[#E5DDD0] border-l-4 border-l-amber-500 p-6 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-[#3A2E22] mt-1">{{ $productsListed ?? 0 }}</p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-[#E5DDD0] border-l-4 border-l-blue-500 p-6 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-[#9E8C78] text-sm font-medium">Recos This Week</p>
                <p class="text-3xl font-bold text-[#3A2E22] mt-1">{{ $recosThisWeek ?? 0 }}</p>
                <p class="text-blue-600 text-sm font-medium mt-1">Prescriptive Insights</p>
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8"
     x-data="{
        ...
        init() {
            window.addEventListener('cafe-owner-notifications-updated', (event) => {
...
>
    <div class="bg-white rounded-2xl shadow-sm border border-[#E5DDD0] p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <a href="{{ route('cafe-owner.my-cafe') }}" class="group bg-[#FAF7F2] hover:bg-[#4A6741] rounded-xl p-4 transition-all duration-200 hover:shadow-lg hover:-translate-y-1 border border-gray-100">
                ...
                <h3 class="font-semibold text-[#3A2E22] group-hover:text-white transition-colors">Manage My Cafe</h3>
                <p class="text-xs text-[#9E8C78] group-hover:text-white/80 transition-colors mt-1">Update cafe profile</p>
            </a>

            <a href="{{ route('cafe-owner.coupon-promos') }}" class="group bg-[#FAF7F2] hover:bg-[#4A6741] rounded-xl p-4 transition-all duration-200 hover:shadow-lg hover:-translate-y-1 border border-gray-100">
                <div class="w-12 h-12 bg-[#4A6741] rounded-lg flex items-center justify-center mb-3 group-hover:bg-white transition-colors">
                    <svg class="w-6 h-6 text-white group-hover:text-[#4A6741] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <h3 class="font-semibold text-[#3A2E22] group-hover:text-white transition-colors">View Promos</h3>
                <p class="text-xs text-[#9E8C78] group-hover:text-white/80 transition-colors mt-1">Digital Coupon Promos</p>
            </a>

            <a href="{{ route('cafe-owner.recommendations') }}" class="group bg-[#FAF7F2] hover:bg-[#4A6741] rounded-xl p-4 transition-all duration-200 hover:shadow-lg hover:-translate-y-1 border border-gray-100">
                <div class="w-12 h-12 bg-[#4A6741] rounded-lg flex items-center justify-center mb-3 group-hover:bg-white transition-colors">
                    <svg class="w-6 h-6 text-white group-hover:text-[#4A6741] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                    </svg>
                </div>
                <h3 class="font-semibold text-[#3A2E22] group-hover:text-white transition-colors">View Recos</h3>
                <p class="text-xs text-[#9E8C78] group-hover:text-white/80 transition-colors mt-1">See weekly insights</p>
            </a>
        </div>
    </div>
...
<style>
    @media (max-width: 767px) {
        .cafe-dashboard-page .cafe-dashboard-header {
        ...
        .cafe-dashboard-page .cafe-dashboard-header h1 {
            font-size: 1.7rem !important;
            line-height: 2rem;
        }

        .cafe-dashboard-page .cafe-dashboard-notif {
            width: 100%;
            justify-content: flex-end;
        }

        .cafe-dashboard-page .cafe-dashboard-notif-menu {
