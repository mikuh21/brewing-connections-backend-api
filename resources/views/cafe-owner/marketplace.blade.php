...
>
    <div @click.away="if (!isCreatingProduct) showCreateModal = false" class="bg-white rounded-xl shadow-2xl w-full max-w-4xl p-3">
        ...
        <form method="POST" action="{{ route('cafe-owner.marketplace.products.store') }}" enctype="multipart/form-data" @submit="isCreatingProduct = true" :aria-busy="isCreatingProduct.toString()" class="grid grid-cols-2 md:grid-cols-3 gap-1.5">
            @csrf
